...
## رفع ارسال تکراری در 0.3.5

اگر SMS Forwarder یک پیام بانکی را بیش از یک بار (مثلاً از مسیر SMS و RCS یا دو فیلتر) ارسال کند، فقط اولین درخواست فاکتور را تأیید می‌کند. درخواست‌های بعدی با نتیجه `duplicate` پاسخ داده می‌شوند، اعلان خطای اضافه ارسال نمی‌شود و کارمزد یا Callback دوباره اجرا نخواهد شد. اثر انگشت تراکنش از متن نرمال‌شده، مبلغ، بانک و پذیرنده ساخته می‌شود و به تغییر نام فرستنده وابسته نیست.


## تغییرات نسخه 0.3.6 — بازطراحی متن‌های ربات

- یکپارچه‌سازی لحن رسمی و حرفه‌ای در پنل پذیرنده و مدیریت
- هماهنگ‌سازی عنوان‌ها، جداکننده‌ها و برچسب دکمه‌ها
- فارسی‌سازی وضعیت فاکتورها و نتایج پردازش پیامک
- خلاصه‌تر و شفاف‌ترشدن اعلان تأیید پرداخت و پیام‌های خطا
- اصلاح راهنمای SMS Forwarder با متغیرهای واقعی `{in-number}` و `{msg}`
