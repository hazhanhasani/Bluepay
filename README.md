# درگاه واسط پرداخت مستقیم — نسخه 0.2.7
...
5. فایل ZIP کامل نسخه 0.2.7 را برای ربات ارسال کنید.
...
## تغییرات نسخه 0.2.7 — آموزش و اتصال
...
{
  "sender": "Bank Mellat",
  "message": "متن کامل پیامک واریز",
  "device_id": "phone-1"
}
```

توکن داخل URL محرمانه است و پیامک دریافت‌شده فقط با فاکتورهای همان پذیرنده تطبیق داده می‌شود.
...
## تغییرات نسخه 0.2.7 — پوشش جامع بانک‌های ایران

- کاتالوگ ۳۴ بانک، مؤسسه و برند بانکی فعال یا قدیمی/ادغام‌شده
- تشخیص نام‌های فارسی، انگلیسی، مخفف‌ها و نام‌های قدیمی فرستنده پیامک
- پشتیبانی از بانک‌های دولتی، خصوصی، تخصصی، قرض‌الحسنه و برندهای دیجیتال
- فهرست صفحه‌بندی‌شده بانک‌ها هنگام ثبت کارت در ربات
- فیلد اختیاری `bank_code` در وبهوک پیامک برای افزایش دقت تشخیص
- API عمومی `GET /api/v1/banks` برای دریافت فهرست کدهای بانکی
- استخراج بهتر مبلغ ریال/تومان، چهار رقم کارت و شماره پیگیری
- جلوگیری از انتخاب مانده حساب به‌عنوان مبلغ تراکنش
- پیامک‌های مبهم یا قالب‌های ناشناخته همچنان برای امنیت به بررسی دستی می‌روند

> قالب پیامک بانک‌ها ممکن است در طول زمان یا بین سرویس‌های مختلف یک بانک تغییر کند. برای فعال‌سازی قطعی خودکار هر قالب، یک نمونه پیامک واریز و برداشت همان بانک را در محیط آزمایشی بررسی کنید.
