...
async def confirm_invoice_paid(
    ...
    # same SELECT contains the nullable OUTER JOIN generated by joinedload()
    # for Invoice.store. selectinload() keeps relationship loading in separate
    # queries, so SMS confirmation works on PostgreSQL while preserving the
    # row lock that prevents two messages from paying one invoice.
    invoice = await session.scalar(
        select(Invoice)
        .where(Invoice.id == invoice_id)
        .options(selectinload(Invoice.card), selectinload(Invoice.merchant), selectinload(Invoice.store))
        .with_for_update()
    )
